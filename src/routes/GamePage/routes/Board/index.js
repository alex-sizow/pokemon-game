...
const BoardPage = () => {
	...
	const [player1, setPlayer1] = useState(() => {
		return Object.values(pokemonContext.selectedPokemons).map(
			(item) => ({
				...item,
				possession: 'blue',
			})
	...
	if (
		Object.keys(pokemonContext.selectedPokemons).length === 0
	) {
	...
	useEffect(() => {
		if (steps === 9) {
			const [count1, count2] = counterWin(
				board,
				player1,
				player2
			);
			...
			if (count1 > count2) {
				pokemonContext.win(true);
				alert('WIN');
			} else if (count1 < count2) {
				pokemonContext.win(false);
				alert('LOSE');
			} else {
				pokemonContext.win(false);
				alert('DRAW');
			...
			<div className={s.board}>
				{board.map((item) => (
					<div
						...
						onClick={() =>
							!item.card && hendleClickBoardPlate(item.position)
						}>
						{item.card && (
